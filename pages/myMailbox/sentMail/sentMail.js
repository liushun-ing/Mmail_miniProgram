...
Page({
	...
	async getMailList() {
		...
		try {
			const res = await api.mail.getSentMail(reqData);
			if (res.code == 2000) {
				let mailList = res.data.sendList;
	...
	*/
	...
	getDetail: function (e) {
		// console.log(e);
		wx.navigateTo({
			url: `/pages/myMailbox/sentMailDetail/sentMailDetail?emailId=${e.currentTarget.dataset.id}`,
		});
	},
	 ...
	 */
	onShow() {
		if (this.data.deleteEmailId) {
			const mailList = this.data.mailList.filter((item) => {
				return item.emailId != this.data.deleteEmailId
			});
			this.data.deleteEmailId = '';
			this.setData({
				mailList: mailList
			})
		}
